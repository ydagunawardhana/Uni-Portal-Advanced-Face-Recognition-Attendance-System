...
const StudentMyAttendance = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "Present":
        return (
          <span className="flex items-center gap-1.5 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 px-3 py-1 rounded-full text-sm font-bold border border-green-100">
            <CheckCircle className="w-4 h-4" /> Present
          </span>
        );
      case "Absent":
        return (
          <span className="flex items-center gap-1.5 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 px-3 py-1 rounded-full text-sm font-bold border border-red-100">
      ...
      default:
        return (
          <span className="bg-gray-200 text-gray-600 dark:text-gray-400 px-3 py-1 rounded-full text-sm font-bold animate-pulse">
            {status}
          </span>
        );
    }
      ...
      {!selectedSubject ? (
        ...
        <>
          <div className="mb-8 flex flex-wrap items-end gap-4">
            ...
            {subjects.length > 0 && subjects[0].level && (
              <div className="flex items-center bg-red-100 dark:bg-red-900/30 text-red-600 px-3 py-1 rounded-xl text-sm font-bold border-2 border-red-200 mb-1">
                {subjects[0].level}
              </div>
            )}
          </div>
          ...
          {isLoading ? (
            <div className="flex justify-center p-12 text-gray-500 dark:text-gray-400 font-medium">
              Loading your subjects...
            </div>
          ) : subjects.length === 0 ? (
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-10 text-center border-2 border-dashed border-red-200 shadow-sm mt-6">
              <div className="w-20 h-20 bg-gray-100 dark:bg-gray-700 text-gray-400 rounded-full flex items-center justify-center mx-auto mb-4">
                <BookOpen className="w-10 h-10 animate-pulse" />
              </div>
              <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">
                No Subjects Found
              </h3>
              <p className="text-gray-500 dark:text-gray-400 max-w-md mx-auto text-sm font-semibold leading-relaxed">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {subjects.map((sub, idx) => (
                <div
                  ...
                  className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-6 cursor-pointer hover:shadow-lg border-2 transition-all group shadow-sm"
                >
                  <div className="flex justify-between items-start mb-4">
                    <div className="w-12 h-12 bg-green-100 dark:bg-green-900/30 text-green-600 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
                  ...
                  <h3
                    className="text-xl font-bold text-gray-900 dark:text-white mb-1 group-hover:text-red-700 dark:text-red-400 transition-colors line-clamp-1"
                  ...
                  <div className="flex items-center gap-4 mt-3">
                    <span className="bg-gray-200 px-2.5 py-1 rounded-xl font-bold text-sm text-gray-700 dark:text-gray-300">
                      {sub.module_code}
                    </span>
        ...
        <div className="animate-in slide-in-from-right-4 duration-300">
          <button
            ...
            className="flex items-center gap-2 text-md font-bold text-gray-500 dark:text-gray-400 hover:text-red-600 transition-colors mb-6 cursor-pointer"
          >
          ...
          <div className="bg-green-600 rounded-2xl p-6 text-white mb-8 shadow-md flex justify-between items-center">
            ...
            <div className="dark:bg-gray-800/10 px-4 py-3 rounded-xl backdrop-blur-sm text-center ">
          ...
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-md overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-left text-gray-600 dark:text-gray-400">
                <thead className="bg-gray-100 dark:bg-gray-700 border-b border-gray-100 dark:border-gray-700 text-sm  text-gray-600 dark:text-gray-400 font-extrabold tracking-wider">
                  <tr>
                            ...
                            {session.status === "Absent" && (
                              session.request_status === "Pending" ? (
                                ...
                                <button
                                  ...
                                  className="text-sm font-bold bg-green-50 dark:bg-green-900/20 text-green-600 px-2.5 py-1 rounded-lg hover:bg-green-100 dark:bg-green-900/30 transition-colors border border-green-200 shadow-sm flex items-center gap-1 cursor-pointer"
                                >
                                  Request Correction
                                </button>
                              )
