...
export default function ManageLecturers() {
  ...
  const handleRegisterLecturer = async (formData: any) => {
    ...
    try {
      ...
      const payload = {
        ...
        assigned_subjects: Array.isArray(formData.assignedSubjects)
      ...
      if (res.ok) {
        toast.success(
          "Lecturer registered successfully! Login credentials have been sent via email.",
          {
            id: toastId,
            duration: 6000,
          }
        ...
        const data = await res
          .json()
          .catch(() => ({ detail: "Malformed server error" }));
  ...
  const handleUpdateLecturer = async (formData: any) => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE}/api/admin/lecturers/${selectedLecturer.id}`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res
          .json()
          .catch(() => ({ detail: "Delete operation failed" }));
        toast.error(parseApiError(data), { id: toastId });
      }
    } catch (error: any) {
  ...
  const handleToggleStatus = (lecturer: Lecturer) => {
    ...
    toast.promise(handleUpdateLecturerStatus(lecturer.id, updatedStatus), {
  ...
  const handleUpdateLecturerStatus = async (id: number, active: boolean) => {
    ...
    const res = await fetch(`${API_BASE}/api/admin/lecturers/${id}`, {
      ...
      body: JSON.stringify({
        ...
        assigned_subjects:
          lecturers.find((l) => l.id === id)?.assigned_subjects || "",
      }),
    });
  ...
  const filteredLecturers = lecturers.filter((lecturer) => {
    ...
    const matchesFaculty =
      facultyFilter === "All Faculties" || lecturer.faculty === facultyFilter;
    ...
    return (
      matchesSearch && matchesFaculty && matchesDepartment && matchesSubject
    );
  });
